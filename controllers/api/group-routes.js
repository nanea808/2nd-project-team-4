...
router.post("/", async (req, res) => {
  ...
  if (req.body.owning_user_id !== req.session.userID) {
    res
      .status(401)
      .json({ message: "Please log in as the user woh will own this group." });
    return;
  }
...
// update a group (change name, add/remove users.)
router.put("/:id", async (req, res) => {
  try {
    ...
    // test if the user is the owner, or a user removing themself
    let owner = false;
        ...
        if (thisGroup.lists[index].user_id == req.body.removedUser) {
          await GroupList.destroy({
            where: {
              ...
              list_id: thisGroup.lists[index].id,
            },
          });
        }
      }
    ...
    if (req.body.newUser && owner) {
      ...
      for (let index = 0; index < thisGroup.users.length; index++) {
        if (thisGroup.users[index].id === user.id) {
          res
            .status(400)
            .json({ message: "This user is already part of your group." });
          return;
        }
      }
    ...
    // change title
    if (req.body.newTitle && owner) {
      const groupData = await Group.update(
        { title: req.body.newTitle },
        { where: { id: req.params.id } }
      );
      res.status(200).json(groupData);
    } else res.status(200).json({ message: `Group users and lists updated.` });
  } catch (err) {
    res.status(500).json(err);
  }
});
  ...
  try {
    ...
    if (thisGroupData.owning_user_id !== req.session.userID) {
      res
        ...
        .json({
          message:
            "This is not your group. Please log in as the group's owner.",
        });
      return;
    }
    await GroupUser.destroy({
    ...
    const groupData = await Group.destroy({
      where: [{ id: req.params.id }, { owning_user_id: req.session.userID }],
    });
